...
import { Star, CheckCircle2, Play, ListOrdered, Maximize, Move, ZoomIn, MousePointer2, Layout, Plus } from 'lucide-react';

interface KnowledgeGraphProps {
  topic: string;
  nodes: KnowledgeNode[];
  onNodeClick: (node: KnowledgeNode) => void;
  onGenerateMore?: (node: KnowledgeNode) => void;
  mode: GraphMode;
  theme?: 'light' | 'dark';
}
...
const KnowledgeGraph: React.FC<KnowledgeGraphProps> = ({ topic, nodes, onNodeClick, onGenerateMore, mode, theme = 'light' }) => {
  ...
  const renderNode = (node: KnowledgeNode) => {
    ...
    const isLeaf = isMindmap 
      ? !nodes.some(n => n.parentId === node.id)
      : nodes[nodes.length - 1].id === node.id;
    ...
    if (isMindmap) {
      if (level === 0) {
        ...
        boxClasses = isDark ? "bg-gray-800 border-l-4 border-l-blue-500 text-gray-100 shadow-sm" : "bg-white border-l-4 border-l-blue-500 text-gray-800 shadow-sm";
      } else {
        nodeSizeClasses = "min-w-[140px] max-w-[200px] py-2 px-4 rounded-lg";
        fontSizeClasses = "text-xs";
        boxClasses = isDark ? "bg-gray-800 border-gray-700 text-gray-300 hover:border-blue-500" : "bg-white border-gray-200 text-gray-700 hover:border-blue-300";
      }
    } else {
      // Linear Mode Default
      nodeSizeClasses = "min-w-[240px] px-6 py-4 rounded-[1.5rem]";
      fontSizeClasses = "text-sm";
      boxClasses = isDark ? "bg-gray-800 border-gray-700 text-gray-100" : "bg-white border-gray-200 text-gray-800";
    }

    // Override colors based on Status
    if (isLocked) {
      boxClasses = isDark ? "bg-gray-900 border-gray-800 text-gray-600 grayscale opacity-70" : "bg-gray-50 border-gray-100 text-gray-400 grayscale opacity-70";
    } else if (isCompleted) {
      if (level === 0 && isMindmap) {
        boxClasses = "bg-green-600 text-white border-green-700 shadow-green-200 shadow-xl";
      } else {
        boxClasses = isDark ? "bg-green-900/30 border-green-800 text-green-400 border-l-4 border-l-green-500" : "bg-green-50 border-green-200 text-green-900 border-l-4 border-l-green-500";
      }
    } else if (isAvailable && level !== 0) {
      boxClasses = isDark ? "bg-gray-800 border-blue-500 border-2 text-white shadow-lg shadow-blue-900/50 ring-4 ring-blue-900/30" : "bg-white border-blue-500 border-2 text-gray-900 shadow-lg shadow-blue-100 ring-4 ring-blue-50/50";
    }
    ...
    return (
      <div 
        ...
        style={{ 
          ...
          left: Math.round(dimensions.width / 2), // Center of screen as origin
      ...
      >
        ...
        <div className="relative group -translate-x-1/2 -translate-y-1/2">
          ...
          >
             ...
             {(!isMindmap || level <= 1) && (
               <p className={`text-[10px] line-clamp-1 leading-relaxed opacity-70 w-full ${isCompleted || (isMindmap && level===0) ? 'text-white/80' : isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                 {node.description}
               </p>
             )}
          </button>
          ...
          {isLeaf && !isLocked && onGenerateMore && (
            <button
              onClick={(e) => { e.stopPropagation(); onGenerateMore(node); }}
              className={`absolute w-8 h-8 ${isDark ? 'bg-gray-800 border-blue-400 text-blue-400 hover:bg-gray-700' : 'bg-white border-blue-500 text-blue-500 hover:bg-blue-50'} border-2 rounded-full flex items-center justify-center hover:scale-110 transition-all shadow-md z-30 ${
                isMindmap 
                  ? '-right-4 top-1/2 -translate-y-1/2' 
                  : 'bottom-[-2.5rem] left-1/2 -translate-x-1/2'
              }`}
              title="生成后续关卡"
            >
              ...
              return (
                <g key={`conn-${node.id}`}>
                  <path 
                    d={d} 
                    fill="none" 
                    stroke={isCompleted ? '#22c55e' : isActive ? '#3b82f6' : isDark ? '#374151' : '#E5E7EB'} 
              ...
              if (index < nodes.length - 1) {
                ...
                return (
                  <line 
                    ...
                    stroke={active ? '#3b82f6' : isDark ? '#374151' : '#E5E7EB'}
                    strokeWidth="4"
  ...
  return (
    <div className={`flex flex-col h-full rounded-[2.5rem] shadow-sm border overflow-hidden relative ${isDark ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'}`}>
      {/* Header Panel */}
      <div className={`p-6 px-8 border-b flex justify-between items-center backdrop-blur-xl sticky top-0 z-50 ${isDark ? 'bg-gray-900/80 border-gray-800' : 'bg-white/80 border-gray-100'}`}>
        <div className="flex items-center gap-4">
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-colors shadow-sm ${isMindmap ? (isDark ? 'bg-blue-900/30 text-blue-400' : 'bg-blue-50 text-blue-600') : (isDark ? 'bg-indigo-900/30 text-indigo-400' : 'bg-indigo-50 text-indigo-600')}`}>
            {isMindmap ? <Layout size={24} /> : <ListOrdered size={24} />}
          </div>
          <div>
            <h2 className={`text-xl font-black tracking-tight ${isDark ? 'text-white' : 'text-gray-900'}`}>{topic || "未命名主题"}</h2>
            <div className="flex items-center gap-3 mt-1">
              <span className={`px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-widest ${isDark ? 'bg-blue-900/50 text-blue-300' : 'bg-blue-100 text-blue-700'}`}>
        ...
        <div className="flex items-center gap-3">
          {isMindmap && (
            <div className={`px-3 py-1.5 rounded-lg border flex items-center gap-2 text-xs font-bold ${isDark ? 'bg-gray-800 border-gray-700 text-gray-400' : 'bg-gray-50 border-gray-100 text-gray-400'}`}>
              <ZoomIn size={12} /> {Math.round(scale * 100)}%
            </div>
          )}
          <button 
            onClick={resetView}
            className={`p-3 rounded-xl transition-all active:scale-95 ${isDark ? 'bg-gray-800 text-gray-400 hover:text-blue-400 hover:bg-gray-700' : 'bg-gray-50 text-gray-400 hover:text-blue-600 hover:bg-blue-50'}`}
            title="重置视图"
          >
            <Maximize size={18} />
          </button>
      ...
      <div 
        ref={containerRef}
        className={`flex-1 relative overflow-hidden select-none touch-none ${isDragging ? 'cursor-grabbing' : isMindmap ? 'cursor-grab' : 'cursor-default'} ${isDark ? 'bg-[#0f1115]' : 'bg-[#F8F9FA]'}`}
      ...
      >
        <div 
          ...
          style={{ transform: `translate(${Math.round(offset.x)}px, ${Math.round(offset.y)}px) scale(${scale})` }}
        >
          {/* Dot Grid Background */}
          <div className="absolute inset-[-4000px] opacity-[0.4] pointer-events-none" 
            style={{ 
              backgroundImage: `radial-gradient(${isDark ? '#374151' : '#CBD5E1'} 1px, transparent 1px)`,
